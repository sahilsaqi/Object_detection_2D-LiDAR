...
def load_model_and_scaler():
    global model, scaler
    model = load_model('/home/aiot/myproject/src/object_detection_2/scripts/models/my_model.keras')
    scaler = joblib.load('/home/aiot/myproject/src/object_detection_2/scripts/models/scaler.pkl')
...
def LaserHandler(data):
    ...
    # Set the header frame and timestamp before publishing
    obs_msg.header.frame_id = data.header.frame_id
    obs_msg.header.stamp = rospy.Time.now()  # Set timestamp
    pub_obj.publish(obs_msg)

    obs_point_msg.header.frame_id = data.header.frame_id
    obs_point_msg.header.stamp = rospy.Time.now()  # Set timestamp
